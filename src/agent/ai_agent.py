...
async def process_input(state: State) -> State:
    ...
    if raw_knowledge and raw_knowledge != "該当するコードナレッジは見つかりませんでした。":
        formatted_knowledge = f"【ローカルナレッジ（ソースコード・ドキュメント）】\n{raw_knowledge}"
    else:
        formatted_knowledge = ""
...
"""
    ...
    return {
        ...
        "input": state["input"],
        "search_results": state["search_results"] if state.get('search_results') else "（ナレッジなし）",
        "output": response
    }
